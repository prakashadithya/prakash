   $date = Get-Date
    ...
    Add-Content \\tsa2\Powershell_Scripts\Outputs\Physical_servers_backup\Physical-server-backup-$date\pre_$server.txt "-----------------------------Hyper-V host info ------------------------------";
    Invoke-Command -ComputerName $server {Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Virtual Machine\Guest\Parameters"} | Out-File  \\tsa2\Powershell_Scripts\Outputs\Physical_servers_backup\Physical-server-backup-$date\pre_$server.txt -Append
